...
class LidarScanner:
    ...
    def _collect_geometry(self, max_range):
        ...
        signature = []
        offset = 0
        # Всё, что крупнее сенсорного горизонта, — это задник (небосвод
        # RenderPipeline масштабирован в 40000), а не поверхность, обо что
        # бьётся луч. Отсекаем по размеру, а не по имени: у скайбокса имя
        # узла "Sphere" в группе "SceneRoot", по нему его не отличить.
        # Заодно страховка от великанского max_range: без отсечки небо
        # превратилось бы в сплошную стену возвратов.
        ...
        for gnp, class_name in _scene_geom_nodes(self.app):
            ...
            if data is None:
                continue
            verts, faces = data
            if float(np.max(verts.max(axis=0) - verts.min(axis=0))) > \
                    backdrop_limit:
                backdrops += 1
                continue
