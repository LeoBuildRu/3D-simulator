...
# На сколько вглубь кузова от НАРУЖНОЙ грани борта отнесено крепление тента,
# в единицах сцены (они же метры: кузов MAZ — 6 м в длину, 2.6 в ширину).
#
# Крепление на самом перегибе (полборта от кромки, ~0.12) не работает: полка
# узкая, ткань свисает с неё в обе стороны, и держат эту позу только сами
# закреплённые вершины — каждая торчит бугром. Отнесённый вглубь якорь стоит
# на плоской опоре, ткань вокруг него подпёрта, и крепление перестаёт быть
# несущим: перегиб через кромку держат вес настила и трение.
#
# Величина абсолютная, а не доля кузова: тент крепят на том же расстоянии от
# кромки и на трёхметровом кузове, и на шестиметровом. Сверху ограничена
# половиной настила (см. _layout_rail), чтобы на коротком настиле якорь не
# уехал за свободный край.
ANCHOR_INSET = 0.5
...
class ClothSimulator:
    ...
    def _layout_rail(self, rail, bounds, rng, placement, cuzov_verts=None,
        ...
        # Куда сядет КРЕПЛЕНИЕ. Отсчёт — от НАРУЖНОЙ грани борта вглубь кузова.
        #
        # Раньше крепление жило на самом перегибе, то есть посреди торца борта:
        # это полборта от кромки, порядка 0.12 в единицах сцены (метрах). Узкая
        # полка, и ткань свисает с неё на ОБЕ стороны — наружу и на груз.
        # Держать такую конфигурацию может только само крепление, поэтому вся
        # нагрузка шла в считанные закреплённые вершины, и каждая торчала
        # бугром: ровно то, что видно на кадрах как «точки слишком заметны».
        #
        # Отнесённое вглубь крепление стоит на ПЛОСКОЙ опоре (верх борта уже
        ...
        # РОВНО НА КРЕПЛЕНИИ: якорь обязан сидеть на высоте опоры. Если подъём
        # ещё продолжается, закреплённый ряд оказывается ниже верха насыпи, то
        # есть внутри груза, а незнаковая коллизия оттуда не вытаскивает.
        ramp = np.clip((vv - cross1)
                       / max(v_anchor - cross1, lie_in * 0.20, _EPS), 0.0, 1.0)
        lateral = np.where(
            vv < cross0,
            clear,
            np.where(vv > cross1,
        ...
        # вниз некуда, там опора.
        #
        # Отсчёт идёт ОТ КРЕПЛЕНИЯ, а не от перегиба: на самом якоре гребень
        # обязан быть нулевым. Иначе закреплённый ряд садится на гребень
        # складки, то есть заведомо выше опоры, и торчит — та же «заметная
        # точка», только теперь из-за затравки, а не из-за узкой полки.
        lie_t = np.clip((vv - v_anchor) / max(total - v_anchor, _EPS), 0.0, 1.0)
        ...
        # ПЕРЕГИБ и КРЕПЛЕНИЕ — теперь разные ряды, и их нельзя путать:
        #   fold_row   — где полотно заламывается через кромку борта. Солвер
        #                отпускает там изгиб (см. CREASE_RELIEF), но НИЧЕГО не
        #                закрепляет: сгиб держится весом настила и трением.
        #   anchor_row — где тент притянут к кузову, на anchor_inset вглубь от
        #                наружной грани, уже на плоской опоре.
        pinned = np.zeros((ny, nx), dtype=bool)
        fold_row = int(np.argmin(np.abs(v - (drop_out + cross_w * 0.5))))
        anchor_row = int(np.argmin(np.abs(v - v_anchor)))
        pinned[anchor_row, :] = True
        self._punch_pins(pinned, rng, row=anchor_row,
        ...
        return {
            ...
            # наружу «крылом» (см. CREASE_RELIEF). Это НЕ ряд крепления.
            "crease_row": fold_row,
            "anchor_row": anchor_row,
            "anchor_inset": anchor_inset,
    ...
    def spawn_random(self, seed=None, placement=None):
        ...
        self.last_params = {
            ...
            "anchor_inset": (None if layout.get("anchor_inset") is None
                             else round(float(layout["anchor_inset"]), 3)),
