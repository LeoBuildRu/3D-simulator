...
RP_PATH = r"C:\Users\larle\toner_project\render_pipeline"
...
class CameraControlGUI(QWidget):
    ...
    def on_model_set_changed(self, model_set_name):
        if model_set_name in self.models_config:
            ...
            info_text += f"Кузов: {os.path.basename(config.get('cuzov', ''))}<br>"
            info_text += f"Наполнитель: {os.path.basename(config.get('napolnitel', ''))}"
    ...
    def generate_perlin_mesh(self):
        success = self.panda_app.generate_perlin_mesh_from_csg()
    ...
    def run_full_process(self):
        ...
        if current_model_set and current_model_set in self.models_config:
            ...
            if success_aabb:
                success_perlin = self.panda_app.generate_perlin_mesh_from_csg()
...
class MyApp(ShowBase):
    ...
    def set_texture_set(self, texture_config):
        ...
        if hasattr(self, 'final_model') and self.final_model is not None:
            self.create_mesh_from_perlin_data()
    ...
    def save_dataset_render(self):
        ...
        for i, volume in enumerate(volumes):
            for pass_num in range(passes_per_volume):
                ...
                success_perlin = self.generate_perlin_mesh_from_csg()
    ...
    def generate_perlin_mesh(self, grid_size=48):
        ...
        if min_bound is None or max_bound is None:
            ...
            max_volumes = {
                "C:/Users/larle/toner_project/models/Scania-Napolnitel.gltf": 24,
                "C:/Users/larle/toner_project/models/Kamaz-Napolnitel.gltf": 10,
                "C:/Users/larle/toner_project/models/Hooper1-Napolnitel.gltf": 48,
                "C:/Users/larle/toner_project/models/Hooper2-Napolnitel.gltf": 88
            }
            ...
            for model in self.loaded_models:
                model_id = id(model)
                if model_id in self.model_paths:
                    if self.Target_Napolnitel in self.model_paths[model_id]:
                        target_model_path = self.model_paths[model_id]
                        break
            
            if target_model_path in max_volumes:
                max_volume = max_volumes[target_model_path]
                volume_ratio = self.Target_Volume / max_volume
        ...
        texture_repeatX = self.current_texture_set.get('textureRepeatX', 1.35)
        texture_repeatY = self.current_texture_set.get('textureRepeatY', 3.2)
        ...
        for y in range(grid_size):
            for x in range(grid_size):
                ...
                u = normalized_u * texture_repeatX  # Умножаем на repeatX
                v = normalized_v * texture_repeatY  # Умножаем на repeatY
        ...
        strength = self.current_texture_set.get('strength', 0.14)
        
        if 'displacement' in self.current_texture_set:
            height_texture_path = self.current_texture_set['displacement']
        elif 'height' in self.current_texture_set:
            height_texture_path = self.current_texture_set['height']
        else:
        ...
        for i, (vx, vy, vz) in enumerate(vertices):
            ...
            # ВАЖНО: Используем ТЕ ЖЕ САМЫЕ UV-координаты, что и для diff текстуры
            # (уже умноженные на textureRepeatX/Y)
            u_repeated = u % 1.0
            v_repeated = v % 1.0
    ...
    def create_mesh_from_perlin_data(self):
        ...
        """Создает меш из сохраненных данных перлина, применяет текстуры и falloff"""
        ...
        # Проверяем наличие target_model_trimesh
        ...
        # Получаем параметры из текущего набора текстур
        texture_repeatX = self.current_texture_set.get('textureRepeatX', 1.35)
        texture_repeatY = self.current_texture_set.get('textureRepeatY', 3.2)
        strength = self.current_texture_set.get('strength', 0.14)
        ...
        # Рассчитываем размеры меша для создания UV-координат
        x_coords = [v[0] for v in vertices]
        y_coords = [v[1] for v in vertices]
        z_coords = [v[2] for v in vertices]
        ...
        # Создаем UV-координаты с учетом textureRepeatX/Y
        texcoords_list = []
        for i, (vx, vy, vz) in enumerate(vertices):
            # Нормализуем координаты от -1 до 1, затем преобразуем к 0-1
            normalized_u = (vx + half_size_x) / size_x
            normalized_v = (vy + half_size_y) / size_y
            
            # Применяем textureRepeat
            u = normalized_u * texture_repeatX
            v = normalized_v * texture_repeatY
            
            texcoords_list.append((u, v))
        
        # Применяем displacement текстуру, если есть
        if 'displacement' in self.current_texture_set:
            height_texture_path = self.current_texture_set['displacement']
        elif 'height' in self.current_texture_set:
            height_texture_path = self.current_texture_set['height']
        ...
        # Применяем displacement к вершинам
        displaced_vertices = []
        for i, (vx, vy, vz) in enumerate(vertices):
            u, v = texcoords_list[i]
            
            # Используем зацикливание текстур
            u_repeated = u % 1.0
            v_repeated = v % 1.0
        ...
        # Применяем falloff к краям меша
        falloff_config = {
            'left': {'width_ratio': 0.20, 'target_offset': -0.15},
            'right': {'width_ratio': 0.20, 'target_offset': -0.15},
            'front': {'width_ratio': 0.15, 'target_offset': -0.2},
            'back': {'width_ratio': 0.25, 'target_offset': -0.2}
        }
        ...
        for side in ['left', 'right', 'front', 'back']:
            if edge_samples[side]:
                ...
                target_heights[side] = vertices[0][2] + falloff_config[side]['target_offset']
        ...
        # Создаем геометрию с новыми вершинами
        format = GeomVertexFormat.getV3n3t2()
        format = GeomVertexFormat.registerFormat(format)
        vdata = GeomVertexData("perlin_recreated", format, Geom.UHStatic)
        ...
        normals = [Vec3(0, 0, 1) for _ in range(len(vertices))]
        for y in range(1, grid_size - 1):
            for x in range(1, grid_size - 1):
                idx = y * grid_size + x
                p = vertices[idx]
        ...
        # Записываем вершины, нормали и UV-координаты
        for i, (vx, vy, vz) in enumerate(vertices):
            vertex_writer.addData3f(vx, vy, vz)
            normal_writer.addData3f(normals[i].x, normals[i].y, normals[i].z)
        ...
        geom_node = GeomNode("recreated_perlin_mesh")
        geom_node.addGeom(geom)
        
        # Создаем Panda3D нод из геометрии
        perlin_np = self.render.attachNewNode(geom_node)
        perlin_np.setPos(0, 0, self.last_best_z if hasattr(self, 'last_best_z') else 0)
        
        # Преобразуем в trimesh для boolean разности
        perlin_model_trimesh = self.panda_to_trimesh(perlin_np)
        
        # Выполняем boolean разность с target_model_trimesh
        try:
            ...
            # Преобразуем результат обратно в Panda3D
            self.final_model = self.trimesh_to_panda(final_result_trimesh)
            
            # Удаляем временный перлин-нод
            perlin_np.removeNode()
        ...
        # Исправляем UV-координаты с учетом textureRepeatX/Y
        self.fix_uv_coordinates_for_final_model(self.final_model, texture_repeatX, texture_repeatY)
        
        # Применяем текстуры
        geom_node = self.final_model.node()
        if geom_node.getNumGeoms() > 0:
            geom = geom_node.getGeom(0)
            ...
            self.final_model.removeNode()
            self.final_model = self.render.attachNewNode(new_geom_node)
            self.final_model.setPos(0, 0, 0)
        
        # Загружаем текстуры
        if 'diffuse' in self.current_texture_set:
            diffuse_path = self.current_texture_set['diffuse']
        elif 'albedo' in self.current_texture_set:
            diffuse_path = self.current_texture_set['albedo']
        else:
            diffuse_path = "textures/stones_8k/rocks_ground_01_diff_8k.jpg"
        
        normal_path = self.current_texture_set.get('normal', 
            "textures/stones_8k/rocks_ground_01_nor_dx_8k.jpg")
        
        roughness_path = self.current_texture_set.get('roughness', None)
        ...
        if diffuse_tex:
            ...
            self.final_model.setTexture(diffuse_tex, 1)
        
        # Применяем normal текстуру
        ...
        if normal_tex:
            ...
            self.final_model.setTexture(normal_stage, normal_tex)
        
        # Применяем roughness текстуру, если есть
        if roughness_path and os.path.exists(roughness_path):
            roughness_tex = self.loader.loadTexture(roughness_path)
            if roughness_tex:
                ...
                self.final_model.setTexture(roughness_stage, roughness_tex)
        
        # Настраиваем материал
        base_material = Material("perlin_recreated_material")
        ...
        self.final_model.setMaterial(base_material, 1)
        
        # Включаем шейдер
        self.final_model.setShaderAuto()
        self.final_model.setTwoSided(True)
        self.final_model.setBin("fixed", 0)
        self.final_model.setDepthOffset(1)
        
        return True

    def generate_perlin_mesh_from_csg(self):
        if hasattr(self, 'test_perlin_mesh') and self.test_perlin_mesh is not None:
        ...
        target_model_trimesh = self.panda_to_trimesh(target_model)

        self.processed_model = self.trimesh_to_panda(target_model_trimesh)
        ...
        target_model_trimesh = self.panda_to_trimesh(target_model_copy)
        ...
        self.loaded_models.append(perlin_base_np)
        ...
        for z in search_points:
            ...
            perlin_model_trimesh_ = self.panda_to_trimesh(perlin_base_np)
            ...
            model_csg_plane_1 = self.trimesh_to_panda(result_csg)
            ...
            material.setShininess(50)
            model_csg_plane_1.setMaterial(material)
            model_csg_plane_1.setShaderAuto()
            model_csg_plane_1.setTransparency(TransparencyAttrib.MAlpha)
            model_csg_plane_1.setAlphaScale(0.7)
            model_csg_plane_1.setTwoSided(True)
            model_csg_plane_1.setScale(1, 1, 1)
            model_csg_plane_1.setPos(0, 0, 0)
            
            if self.current_display_model is not None:
                self.current_display_model.removeNode()
            
            self.current_display_model = model_csg_plane_1
            self.current_display_model.reparentTo(self.render)
            
            volume = self.calculate_mesh_volume(self.current_display_model)
        ...
        if len(best_points) >= 2:
            ...
            vol1, err1 = evaluate_at_z(x1)
            vol2, err2 = evaluate_at_z(x2)
        ...
        perlin_model_trimesh = self.panda_to_trimesh(perlin_detailed_np)
    ...
    def fix_uv_coordinates_for_final_model(self, node, texture_repeatX=1.0, texture_repeatY=1.0):
        ...
        for i in range(geom_node.getNumGeoms()):
            ...
            for j in range(num_vertices):
                if not vertex.isAtEnd():
                    pos = vertex.getData3()

                    if size_x > 0 and size_y > 0:
                        u = ((pos.x - min_bound.x) / size_x) * texture_repeatX
                        v = ((pos.y - min_bound.y) / size_y) * texture_repeatY
                    else:
                        u = v = 0.0
    ...
    def load_model_set(self, config, model_set_name):
        ...
        if 'other' in config and os.path.exists(config['other']):
            other_model = self.load_gltf_model(config['other'])
            if other_model:
                models_loaded.append('other')
                self.current_other_path = config['other']
        
        if 'cuzov' in config and os.path.exists(config['cuzov']):
            cuzov_model = self.load_gltf_model(config['cuzov'])
            if cuzov_model:
                models_loaded.append('cuzov')
                self.Target_Cuzov = os.path.basename(config['cuzov'])
                self.current_cuzov_path = config['cuzov']
        
        if 'napolnitel' in config and os.path.exists(config['napolnitel']):
            napolnitel_model = self.load_gltf_model(config['napolnitel'])
            if napolnitel_model:
                napolnitel_model.hide()
                models_loaded.append('napolnitel')
                self.Target_Napolnitel = os.path.basename(config['napolnitel'])
                self.current_napolnitel_path = config['napolnitel']
